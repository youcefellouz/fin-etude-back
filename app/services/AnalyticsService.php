    ...
    private function detectLowStock(array &$alerts): void
    {
        ...
        foreach ($low as $product) {
            ...
            $alerts[] = SmartAlert::create([
                ...
                'title'               => "low stock: {$article->name}",
                'message'             => "Only {$product->total_quantity} unit(s) left in stock",
    ...
    private function detectSalesDrop(array &$alerts): void
    {
        ...
        foreach ($products as $analytics) {
            ...
            $alerts[] = SmartAlert::create([
                ...
                'severity'            => 'medium',
                'title'               => "Sales Drop: {$article->name}",
                'message' => "Decline of " . abs(round($analytics->sales_trend)) . "%",
                'recommended_action'  => 'think about a promotional offer',
                'importance_score'    => 70,
                'related_entity_type' => 'article',
    ...
    private function detectAtRiskCustomers(array &$alerts): void
    {
        ...
        foreach ($atRisk as $analytics) {
            ...
            $alerts[] = SmartAlert::create([
                ...
                'severity'            => 'medium',
                'title'               => "Customer Churn: {$user->name}",
                'message' => "Has not purchased in {$analytics->days_since_last_order} days",
                'recommended_action'  => 'send a special offer',
                'importance_score'    => 65,
